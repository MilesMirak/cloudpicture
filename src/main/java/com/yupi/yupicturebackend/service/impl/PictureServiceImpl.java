...
//            }
        ...
        //上传图片，得到图片信息
        //按照用户id划分目录   按照空间划分
        String uploadPathRrefix=null;
        if (spaceId==null){
            uploadPathRrefix = String.format("public/%s",loginUser.getId());
        }else {
            uploadPathRrefix = String.format("space/%s",spaceId);
        }
        ...
        UpLoadPictureResult upLoadPictureResult = pictureUploadTemplate.uploadPicture(inputSource, uploadPathRrefix);
        //构造要入库的图片信息
        ...
        picture.setPicWidth(upLoadPictureResult.getPicHeight());
...
//        this.clearPictureFile(oldPicture);
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count>=30,ErrorCode.PARAMS_ERROR,"最多抓取三十条");
        //抓取内容
        ...
        for (Element imgElement : imgElementList) {
            ...
            //处理图片地址，防止转义或者和对象存储冲突的问题    把url从？开始后面全部不要
            ...
            try {
                ...
                log.info("上传图片成功,id为" + pictureVO.getId());
                uploadCount++;
            } catch (Exception e) {
                log.error("上传图片失败", e);
                continue;
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        fillPictureNameRole(pictureList,nameRule);
        //5 操作数据库进行批量更新
    ...
    public CreateOutPaintingTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, User loginUser) {
        ...
        //判断pictureId是否为空，如果为空执行orElseThrow()括号里面的抛出异常
     ...
     * @param nameRule
     */
    private void fillPictureNameRole(List<Picture> pictureList, String nameRule) {
        ...
        for (Picture picture : pictureList) {
            try{
                //正则表达式左括号记得转义
                String pictureNmae = nameRule.replaceAll("\\{序号}",String.valueOf(count++));
                picture.setName(pictureNmae);
            }catch (Exception e){
                log.error("名称解析错误",e);
